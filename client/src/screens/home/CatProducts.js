  ...
  return (
    <>
        ...
        <div className="my-container my-10">
          {isFetching ? (<div className="flex flex-wrap -mx-4 mb-10">
              {[1,2,3,4].map((item) => (
                  <div className="w-6/12 sm:w-4/12 md:w-3/12 lg:4/12 xl:w-3/12 p-4 " key={item}>
                      <Skeleton>
          ...
          <Pagination page={parseInt(page)} perPage={data.perPage} count={data.count} path={`cat-products/${name}`} theme="light" />
          </>) : <p className="alert-danger flex items-center justify-center text-lg">No products found in #{name} category.</p>}
        </div>
    </>
  )
}
